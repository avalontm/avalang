...
REM (real ANTLR4 parser, not the stub) and for build_studio.bat's
REM ai_agent plugin to link:
REM   1. Clones + bootstraps vcpkg (unless already present)
REM   2. vcpkg install antlr4 using the x64-windows-static-md triplet
REM   3. vcpkg install curl using the same triplet (needed by Ava
REM      Studio's ai_agent plugin, see PLAN_agente_ia_openrouter.md)
REM   4. vcpkg install libffi using the same triplet (needed for real
REM      `extern` / native FFI calls -- e.g. libraries/mysql/)
REM   5. Downloads the ANTLR4 tool jar into third_party\ (only needed at
REM      CMake-configure time, to regenerate the C++ parser from the .g4)
REM   6. Persists VCPKG_ROOT / AVA_VCPKG_TRIPLET via setx, then runs
REM      build.bat
REM
REM Usage:
...
if not defined VCPKG_DIR set "VCPKG_DIR=%~dp0..\vcpkg"
...
REM --- 4b. libcurl, needed by Ava Studio's ai_agent plugin ----------------
REM PLAN_agente_ia_openrouter.md Fase 1: el plugin ai_agent habla contra
REM OpenRouter con libcurl (ver runtime/avastudio/plugins/ai_agent/
REM CMakeLists.txt, find_package(CURL REQUIRED)) -- a diferencia de
REM antlr4, esto NO tiene fallback: sin esto build_studio falla directo
REM en el configure step con "Could NOT find CURL". Se instala acá,
REM junto al resto de las libs de vcpkg, aunque avalang.dll en sí no la
REM use -- este script instala TODO lo que el repo necesita de vcpkg,
REM no solo lo del target avalang.
echo.
echo Installing curl:%AVA_TRIPLET% via vcpkg ...
echo ^(needed by Ava Studio's ai_agent plugin, see build_studio^)
"%VCPKG_DIR%\vcpkg.exe" install curl:%AVA_TRIPLET%
if errorlevel 1 (
    echo [ERROR] "vcpkg install curl:%AVA_TRIPLET%" failed. See output above.
    exit /b 1
)

REM --- 4c. libffi, needed for real `extern` (native FFI) calls ------------
REM cmake/FindLibFFI.cmake SI tiene fallback (avalang compila igual sin
REM esto -- ver runtime/avalang/CMakeLists.txt AVA_HAVE_LIBFFI), pero sin
REM libffi todo `extern "algo.dll" as X func f(...) ... end` falla en
REM tiempo de ejecucion con "no se puede invocar -- este build se
REM compilo sin libffi" (vm_extern.cpp) -- lo pisa cualquier libreria de
REM terceros que use extern, como libraries/mysql/. Se instala aca para
REM que quede resuelto desde el primer build, en vez de que cada usuario
REM lo descubra recien al chocar con ese error en runtime.
echo.
echo Installing libffi:%AVA_TRIPLET% via vcpkg ...
echo ^(needed for real 'extern' / native FFI calls -- e.g. libraries/mysql^)
"%VCPKG_DIR%\vcpkg.exe" install libffi:%AVA_TRIPLET%
if errorlevel 1 (
    echo [ERROR] "vcpkg install libffi:%AVA_TRIPLET%" failed. See output above.
    exit /b 1
)
