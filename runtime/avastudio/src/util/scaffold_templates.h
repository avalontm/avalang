...
// The three file kinds "Generar" (§3.1) can produce inside an existing
// project. Deliberately just these -- the wizard for a brand new *project*
// (main.ava + folder) is Fase 6, a separate flow.
enum class ScaffoldKind { kClass, kScreen, kInterface };

// ".ava" for kClass and kInterface, ".avaui" for kScreen. Explorer forces
// this extension on the created file regardless of what the user typed in
// the name field, so the boilerplate written to disk always matches what
// the extension says it is -- see BuildScaffoldContent below for why that
// pairing matters.
...
//                     example.ava; for kInterface, used as the interface
//                     name instead -- interfaces have no constructor).
//                     Falls back to "NewClass" if base_name sanitizes to
//                     nothing (e.g. the user typed only punctuation).
