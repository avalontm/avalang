...
AppManifest DefaultAppManifest() {
    // Empty on purpose: AvaUI renders every component natively (see
    // config/app_manifest.h) through the same LayoutEngine that both
    // GdiRenderer (desktop) and HTMLRenderer (web) consume, so a
    // project doesn't need CSS or Tailwind just to render. Nothing is
    // scaffolded or defaulted here -- if a project later wants
    // hand-written CSS on top of the native render, or a genuine
    // third-party JS library, it declares that itself in its own
    // app.ava with `import "css/whatever.css"` / `import
    // "js/some-lib.js"`; ResourceImport/BuildHeadTags/BuildBodyEndTags
    // already support that opt-in path, this function just stops
    // assuming it.
