...
bool AppConfig::Load(HostOptions& options, std::string& outError) {
    ...
    if (data.contains("sessionTtlSeconds") && data["sessionTtlSeconds"].is_number_integer())
        options.sessionTtlSeconds = data["sessionTtlSeconds"].get<int>();

    if (data.contains("workerThreads") && data["workerThreads"].is_number_integer())
        options.workerThreads = data["workerThreads"].get<int>();
