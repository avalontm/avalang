...
// RunScript / avaui page render -- see web/app.cpp.
...
public:
    ...
    // RunScript/page render; overwrites whatever the previous request
    ...
    // exposing the core's RouteParameter type, same boundary rule this
    // file follows everywhere (avalang.h is the only thing that crosses).
    struct RouteParam {
        std::string name;
        bool optional = false;
    ...
    // text -- lighter than parsing the file's full component tree, which
    // this doesn't need. Used by web/router.h to build its route
    ...
    // Parses `text` as a .avaui file purely to check it's well-formed
    // (used by `avahost build`, cli_commands.cpp's CmdBuild) -- the
    // component tree/state/imports/routes it parses into are discarded
    // immediately, only the ok/error result is kept. Lighter-weight
    // alternative to what ParseAvaUiFile used to return now that
    // nothing else in AvaHost needs the parsed tree at request time
    // (the avaui pipeline does its own parsing). Returns true and
    // leaves `outError` untouched on success.
    ...
    // These methods are the only place outside RunScript* that reach
    // into this VM's globals -- kept here rather than in the
    // avahost/src/rendering bridges that drive them (ui_vm_state_bridge.h,
    // ui_vm_event_bridge.h) per this file's own header comment:
    // RuntimeHost is the one layer allowed to touch
    // ava_compile/ava_run/ava_set_global directly.

    // Binds `stateJson` (shape: ava::ui::StateToJson's output, i.e.
    // {"key":"raw text value", ...}) as globals on this VM, inferring
    // bool/number/string the same way
    ...
    // the avaui pipeline) `state` block -- it always overwrites whatever
    ...
    // Studio's EvalPropertyExpr. Used by ui_vm_state_bridge.cpp's
    // VmStateBridge::EvalIdentifier so rendered markup reflects current
    // state (including expressions like `"Count: " + counter`) instead
    // of raw source text.
