...
Un `click = ...` explícito también acepta un call completo con argumentos
(`click = OnAdd(Player)`, `click = OnAdd("Raul", 38, data)`) -- el
auto-bind en sí sigue generando solo nombres pelados. Ver
`AVAUI_EVENT_CALL_ARGS.md` para el detalle de cómo se resuelve contra
la VM.
